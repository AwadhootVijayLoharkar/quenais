...
Plots generated:
  ...
  2. SQD convergence curve
  3. Active orbital deviations
  4. Bath singular values
  5. Löwdin population heatmap
  6. Geometry scan PES  (classical + optional quantum curve)
  7. Summary plot + CSV  (all simulation details)

Usage:
  python step4_visualize.py [--no-scan] [--no-quantum-scan]
"""
...
parser.add_argument("--no-scan",         action="store_true",
                    help="Skip geometry scan entirely")
parser.add_argument("--no-quantum-scan", action="store_true",
                    help="Skip quantum curve in geometry scan")
...
# ═══════════════════════════════════════════════════════════════════════════════
# Plot 3 — Active Orbital Deviations
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# Quantum scan helper — runs a lightweight DMET+SQD at a single geometry
# ═══════════════════════════════════════════════════════════════════════════════

def _quantum_energy_at_geometry(new_geom):
    """
    Single-point quantum (DMET+SQD) energy at the given geometry.
    Fast mode: config.QUANTUM_SCAN_SHOTS shots, config.QUANTUM_SCAN_ITERS iters.
    """
    ...
    # ── Mean-field  (RHF for singlet, UHF otherwise — faster + more stable) ──
    try:
        is_restricted = (config.SPIN == 0)
    ...
    # Alpha MO coefficients (works for both RHF and UHF)
    mc_raw = np.asarray(mf_s.mo_coeff)
    if mc_raw.ndim == 3:          # UHF: shape (2, n_ao, n_mo)
        mo_coeff_s = mc_raw[0]
    else:                          # RHF: shape (n_ao, n_mo)
        mo_coeff_s = mc_raw
    ...
    try:
        ...
        # Clamp MO indices to valid range for this geometry
        valid_mo = [i for i in mo_list if i < mo_coeff_s.shape[1]]
        if len(valid_mo) < 2:
            warnings.warn("  [QScan] fewer than 2 valid MOs.", RuntimeWarning)
            return None

        C_imp_s = mo_coeff_s[:, valid_mo]          # (n_ao, n_imp)
        Q_imp_s = S_sqrt_s @ C_imp_s               # (n_ao, n_imp)

        # Total density matrix — handle RHF (2D) and UHF (3D or tuple)
        ...
        if isinstance(dm_raw, (tuple, list)):
            ...
            if dm_raw.ndim == 3:          # UHF ndarray (2, n_ao, n_ao)
                dm_ao_s = dm_raw[0] + dm_raw[1]
            else:                          # RHF ndarray (n_ao, n_ao)
                dm_ao_s = dm_raw

        dm_lo_s = S_sqrt_s @ dm_ao_s @ S_sqrt_s    # Löwdin basis
        ...
        # Bath selection
        n_bath_max = min(n_imp, max(0, config.MAX_EMBED_ORBS - n_imp))
        n_above    = int(np.sum(sv_s > config.BATH_TOLERANCE))
        n_bath_s   = min(n_bath_max, max(0, n_above))

        if n_bath_s > 0:
            Q_emb_s = np.hstack([Q_imp_s, U_env_s[:, :n_bath_s]])   # both 2D ✓
        else:
            Q_emb_s = Q_imp_s.copy()

        n_emb_s = len(valid_mo) + n_bath_s
        C_emb_s = S_invsqrt_s @ Q_emb_s            # (n_ao, n_emb_s)

        # Core mean-field potential
    ...
    try:
        ...
        # SU2 ansatz with HF initialisation
        ...
        ansatz_c = efficient_su2(
            n_qubits_s, reps=2, entanglement="linear",   # linear = shallower
        ...
        shots_s = (config.QUANTUM_SCAN_SHOTS if config.QUANTUM_SCAN_FAST
                   else config.N_SHOTS)
        iters_s = (config.QUANTUM_SCAN_ITERS if config.QUANTUM_SCAN_FAST
                   else config.SQD_ITERS)

        # Sample via MPS backend
        ...
        if bsm_s.shape[0] == 0:
            warnings.warn("  [QScan] no valid configs after filter.", RuntimeWarning)
            return None
        ...
        for it_s in range(iters_s):
            ...
            e_emb_s, _, avg_occs_s, _ = solve_fermion(
                bsm_s, hcore=h1e_s, eri=h2e_s, open_shell=False, spin_sq=0.0,
            )
...
# ═══════════════════════════════════════════════════════════════════════════════
# Plot 6 — Geometry Scan PES  (classical + quantum comparison)
# ═══════════════════════════════════════════════════════════════════════════════

def plot_geometry_scan():
    """
    Potential energy surface: classical method curve + quantum DMET+SQD curve.

    Classical scan: fast, one HF/MP2/CCSD per geometry.
    Quantum scan  : slower, full DMET+SQD per geometry in fast mode.
                    Controlled by config.QUANTUM_SCAN and --no-quantum-scan flag.

    Both curves are plotted on the same axes for direct comparison.
    The quantum curve will typically lie below the classical curve,
    showing the additional correlation energy captured by the quantum solver.
    """
    ...
    print(f"\n  ── Classical scan ({config.SCAN_METHOD}) ─────────────────────────")
    ...
    for d in config.SCAN_DISTANCES:
        ...
        try:
            mol_s = gto.M(atom=new_geom, basis=config.BASIS,
                          charge=config.CHARGE, spin=config.SPIN, verbose=0)
            mf_s  = (scf.RHF(mol_s) if config.SPIN == 0 else scf.UHF(mol_s))
            mf_s.verbose = 0
            mf_s.kernel()
            ...
            warnings.warn(f"Classical scan failed at d={d:.3f} Å: {ex}", RuntimeWarning)
    ...
    # ── Quantum scan ──────────────────────────────────────────────────────────
    run_quantum = (config.QUANTUM_SCAN and not args.no_quantum_scan
    ...
    if run_quantum:
        mode_str = ("fast" if config.QUANTUM_SCAN_FAST else "full")
        print(f"\n  ── Quantum scan (DMET+SQD, {mode_str} mode) ──────────────────")
        print(f"     shots={config.QUANTUM_SCAN_SHOTS if config.QUANTUM_SCAN_FAST else config.N_SHOTS}, "
              f"iters={config.QUANTUM_SCAN_ITERS if config.QUANTUM_SCAN_FAST else config.SQD_ITERS}")
        print(f"     Estimated time: "
              f"~{len(config.SCAN_DISTANCES) * (2 if config.QUANTUM_SCAN_FAST else 10)} min")
        ...
        reason = ("QUANTUM_SCAN=False" if not getattr(config, "QUANTUM_SCAN", False)
                  else "--no-quantum-scan flag" if args.no_quantum_scan
                  else "Step 1/2 data missing")
        print(f"\n  [Skip]")

    # ── Relative energies ─────────────────────────────────────────────────────
    # Use a common reference: minimum of whichever curves are available
    all_energies  = list(cl_energies)
    if qm_energies:
        all_energies += qm_energies
    e_ref = min(all_energies)

    cl_rel = (cl_energies - e_ref) * config.HARTREE_TO_KCAL_MOL
    qm_rel = (np.array(qm_energies) - e_ref) * config.HARTREE_TO_KCAL_MOL \
    ...
    ax.plot(cl_distances, cl_rel, "o-", color="#4C72B0",
            ...
            label=f"Classical: {config.SCAN_METHOD}  (eq={d_eq_cl:.3f} Å)")

    if len(qm_rel) > 0:
        ax.plot(qm_distances, qm_rel, "s--", color="#DD8452",
                linewidth=2, markersize=7,
                label=(f"Quantum: DMET+{config.QUANTUM_SOLVER.upper()} "
                       f"({'fast' if config.QUANTUM_SCAN_FAST else 'full'})  "
                       f"(eq={d_eq_qm:.3f} Å)"))
    ...
    # Annotate equilibrium distances
    for d_eq, color, label in [
        (d_eq_cl, "#4C72B0", config.SCAN_METHOD),
...
# ═══════════════════════════════════════════════════════════════════════════════
# Plot 7 + CSV — Full Simulation Summary
# ═══════════════════════════════════════════════════════════════════════════════

def plot_and_save_summary(scan_data=None):
    """
    Collects every key quantity from all pipeline steps and outputs:

      results/simulation_summary.csv   — machine-readable table of all values
      results/plots/plot7_summary.png  — visual summary dashboard (4 panels)

    Panels:
      A. Energy ladder: all methods ranked by energy (absolute and vs HF)
      B. Active space & correlation strength indicators
      C. Embedding parameters (qubits, bath SVs, sv² coverage)
      D. SQD convergence miniature (final energy vs HF/MP2)
    """
    ...
    data["basis"]          = config.BASIS
    data["charge"]         = config.CHARGE
    data["spin_2s"]        = config.SPIN
    data["n_atoms"]        = config.N_ATOMS
    data["atom_symbols"]   = " ".join(config.ATOM_SYMS)

    # Step 0: classical energies
    e_hf = None
    if step0 is not None:
        e_hf = step0["methods"].get("HF", {}).get("energy")
        data["E_HF_Ha"]    = e_hf
        ...
        for method in ["MP2", "CCSD", "CCSD_T", "CASSCF", "NEVPT2"]:
            ...
            if e is not None and e_hf is not None:
                data[f"E_{method}_vs_HF_kcal"] = (
                    (e - e_hf) * config.HARTREE_TO_KCAL_MOL
                )

    # Step 1: active space / ASF
    if step1 is not None:
        ...
        data["mp2_corr_Ha"]        = step1.get("mp2_energy", 0) - step1.get("uhf_energy", 0)
        data["homo_lumo_gap_eV"]   = (step1.get("indicators", {})
                                      .get("homo_lumo_gap_eV"))
        data["s2"]                 = (step1.get("indicators", {})
                                      .get("s2"))
        data["has_tm"]             = (step1.get("indicators", {})
                                      .get("has_tm"))

    # Step 2: embedding / DMET
    ...
    if scan_data is not None:
        data["scan_method"]          = config.SCAN_METHOD
        data["scan_atom_pair"]       = str(config.SCAN_ATOM_PAIR)
    ...
    # Config snapshot
    data["config_quantum_solver"]  = config.QUANTUM_SOLVER
    data["config_ansatz"]          = config.ANSATZ
    data["config_mapping"]         = config.FERMION_TO_QUBIT
    data["config_backend"]         = config.BACKEND
    data["config_basis"]           = config.BASIS
    data["config_n_shots"]         = config.N_SHOTS
    data["config_sqd_iters"]       = config.SQD_ITERS

    # ── Write CSV ─────────────────────────────────────────────────────────────
    csv_path = os.path.join(config.RESULTS_DIR, "simulation_summary.csv")
    ...
    # ── Build summary plot (4-panel dashboard) ────────────────────────────────
    fig = plt.figure(figsize=(16, 11))
    fig.suptitle(f"Simulation Summary — {molecule} / {config.BASIS}",
                 fontsize=14, fontweight="bold", y=0.98)

    gs = fig.add_gridspec(2, 2, hspace=0.42, wspace=0.35)
    ax_a = fig.add_subplot(gs[0, 0])   # Energy ladder
    ax_b = fig.add_subplot(gs[0, 1])   # Correlation indicators
    ax_c = fig.add_subplot(gs[1, 0])   # Embedding parameters
    ax_d = fig.add_subplot(gs[1, 1])   # SQD convergence miniature

    # ── Panel A: Energy Ladder ────────────────────────────────────────────────
    energy_methods = {}
    method_colors  = {}
    palette = {
        "HF"    : "#4C72B0", "MP2"    : "#55A868",
        "CCSD"  : "#C44E52", "CCSD_T" : "#8172B2",
        "CASSCF": "#937860", "NEVPT2" : "#DA8BC3",
        "Quantum": "#DD8452",
    }
    ...
    if energy_methods and e_hf is not None:
        ...
        vals_a   = [(energy_methods[m] - e_hf) * config.HARTREE_TO_KCAL_MOL
                    for m in labels_a]
        colors_a = [method_colors.get(m.split("\n")[0], "#7F7F7F") for m in labels_a]
    ...
    # ── Panel B: Correlation Indicators ───────────────────────────────────────
    ax_b.axis("off")
    ...
    if step1 is not None:
        rows_b = [
            ...
            ("HOMO-LUMO gap",    f"{step1.get('indicators',{}).get('homo_lumo_gap_eV','N/A'):.3f} eV"
                                 if isinstance(step1.get("indicators",{}).get("homo_lumo_gap_eV"), float)
                                 else "N/A"),
            ("⟨S²⟩",            f"{step1.get('indicators',{}).get('s2','N/A'):.4f}"
    ...
    if rows_b:
        tbl = ax_b.table(
            cellText  = [[k, str(v)] for k, v in rows_b],
    ...
    if step2 is not None:
        rows_c = [
            ...
            ("Fermion mapping", config.FERMION_TO_QUBIT.upper()),
            ("Ansatz",          config.ANSATZ.upper()),
            ("Backend",         config.BACKEND.upper()),
            ("Shots",           config.N_SHOTS),
            ("SQD iters",       config.SQD_ITERS),
        ]
        if step3 is not None and step3.get("energy") is not None:
            rows_c.append(
                ("Final E (quantum)", f"{step3['energy']:.8f} Ha")
            )

    if rows_c:
        tbl_c = ax_c.table(
            cellText  = [[k, str(v)] for k, v in rows_c],
    ...
    # ── Panel D: SQD Convergence Miniature ────────────────────────────────────
    ...
    if step3 is not None and len(step3.get("iterations", [])) >= 2:
        ...
        ax_d.axhline(uhf_d, color="#4C72B0", linewidth=1.5,
                     linestyle="--", label=f"UHF", alpha=0.8)
        ax_d.axhline(mp2_d, color="#55A868", linewidth=1.5,
                     linestyle="-.", label=f"MP2", alpha=0.8)
        ...
        if len(y_d) >= 2:
            delta = y_d[-1] - y_d[0]
            ax_d.annotate(
                f"ΔE = {delta*config.HARTREE_TO_KCAL_MOL:+.2f} kcal/mol\n"
    ...
    path7 = os.path.join(config.PLOTS_DIR, "plot7_summary.png")
...
print(f"\n[Step 4] All outputs saved to: {config.PLOTS_DIR}/")
...
print(f"  plot6_geometry_scan.png      — PES: classical + quantum comparison")
print(f"  plot7_summary.png            — Full simulation summary dashboard")
print(f"  ../simulation_summary.csv    — All simulation quantities (CSV)")